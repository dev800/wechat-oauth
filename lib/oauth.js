...
var processToken = function (that, callback) {
  ...
  return function (err, data, res) {
    ...
    // 存储token
    that.saveToken(that, data.openid, data, function (err) {
      console.log('processToken: saveToken -> ', err, data)
...
var OAuth = function (options) {
  ...
  this.isMiniProgram = options.isMiniProgram;
  this.provider = options.provider;
  this.providerMedia = options.providerMedia;
  this.providerPlatform = options.providerPlatform;

  // token的获取和存储
  this.store = {};

  this.getToken = options.getToken || function (oauth, openid, callback) {
    callback(null, this.store[openid]);
  };

  if (!options.saveToken && process.env.NODE_ENV === 'production') {
    console.warn('Please dont save oauth token into memory under production');
  }

  this.saveToken = options.saveToken || function (oauth, openid, token, callback) {
 ...
 * Examples:
 * ```
 * api.decryptMiniProgramData({encryptedData, iv}, callback);
 * ```
 * Callback:
 *
 ...
 */
OAuth.prototype.decryptMiniProgramData = function (options) {
...
OAuth.prototype.getUser = function (options, callback) {
  ...
  this.getToken(this, options.openid, function (err, data) {
    if (err) {
      return callback(err);
    }
...
OAuth.prototype.getUserByCode = function (options, callback) {
  ...
  if (this.isMiniProgram) {
    this.getSessionKey(code, function (err, result) {
      ...
      var user;
      try {
        user = that.decryptMiniProgramData({
          sessionKey,
